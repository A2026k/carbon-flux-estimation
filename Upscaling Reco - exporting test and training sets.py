...
file_path = r'J:\Final\ER_NDVI_DAY.xlsx'  # Replace with your Excel file path
data = pd.read_excel(file_path)

# Extract features, target variable, and columns to retain (if any)
X = data[['Feature3', 'Feature6', 'Feature8', 'Feature11']].copy()  # Features for modeling
y = data['Target'].copy()  # Target variable

# Check if 'station' and 'time' columns exist
keep_cols = []
...
# Split dataset - split features, target, and metadata (if available)
...
model_file_path = r'F:\Thesis\TPMFD\Model_Accuracy\ER\best_random_forest_model_ER.pkl'
scaler_file_path = r'F:\Thesis\TPMFD\Model_Accuracy\ER\standard_scaler_ER.pkl'
joblib.dump(best_model, model_file_path)
joblib.dump(scaler, scaler_file_path)
print(f'Best model saved to {model_file_path}')
print(f'StandardScaler saved to {scaler_file_path}')

# Create DataFrame for training set results
if not metadata.empty:
    result_train_df = pd.DataFrame({
        'station': meta_train['station'].values if 'station' in meta_train.columns else None,
        'time': meta_train['time'].values if 'time' in meta_train.columns else None,
        'True_ER_train': y_train.values,
        'Predicted_ER_train': y_pred_train_best
    })
else:
    result_train_df = pd.DataFrame({
        'True_ER_train': y_train.values,
...
if not metadata.empty:
    result_test_df = pd.DataFrame({
        'station': meta_test['station'].values if 'station' in meta_test.columns else None,
        'time': meta_test['time'].values if 'time' in meta_test.columns else None,
        'True_ER_test': y_test.values,
        'Predicted_ER_test': y_pred_test_best
    })
else:
    result_test_df = pd.DataFrame({
        'True_ER_test': y_test.values,
...
output_train_file = r'F:\Thesis\TPMFD\Model_Accuracy\ER\predicted_ER_train.xlsx'
...
output_test_file = r'F:\Thesis\TPMFD\Model_Accuracy\ER\predicted_ER_test.xlsx'
...
X_new = new_data[['Feature3', 'Feature6', 'Feature8', 'Feature11']].copy()
...
if all(col in new_data.columns for col in keep_cols):
    result_df = pd.DataFrame({
        'station': new_data['station'].values if 'station' in new_data.columns else None,
        'time': new_data['time'].values if 'time' in new_data.columns else None,
        'ER_DT': y_pred_new
    })
else:
    result_df = pd.DataFrame({
...
output_file = r'F:\Thesis\TPMFD\Model_Accuracy\ER\predicted_ER_DT.xlsx'
